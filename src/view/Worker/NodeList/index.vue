<template>
	<div class="xl:w-72 w-48 flex-shrink-0 border-r border-gray-200 dark:border-gray-800 h-full overflow-y-auto lg:block hidden p-5">
		<div class="text-xs text-gray-400 tracking-wider">{{ props.nodeType }}</div>
		...
		<div class="space-y-4 mt-3">
			<button v-for="[url, item] in filterNodeList" draggable="true" @dragstart="dragstart($event, url)" :data-url="url" class="bg-white p-3 w-full flex flex-col rounded-md dark:bg-gray-800 shadow focus:outline-none">
...
const filterNodeList = computed(() => {
	if(!props.nodeType) return nodeList;
	const list = [...nodeList.entries()].filter(([url, item]) => item.nodeType === props.nodeType);
	return new Map(list);
});
