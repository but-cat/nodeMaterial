<template>
	<node-block :position="`${node!.x},${node!.y}`" :id="`block-${node.uuid}`" @click="emitActiveNode" @connected-down="emit('down', $event)" @connected-up="emit('up')" @connected-update="positionUpdate" :key="`node-${node.uuid}`" :class="isActive ? ['ring-2', 'ring-primary-300'] : []" ref="nodeBlock" class="w-64 bg-white flex flex-col text-gray-700 dark:text-gray-200 rounded-md dark:bg-gray-800 shadow-lg">
		<div class="p-3 h-20 relative">
			<div class="node__holder mb-2 flex align-content">
				<div class="flex xl:flex-row flex-col items-center font-medium text-gray-900 dark:text-white w-full">{{ node.name }} - {{ node.refIndex }}</div>
				<div :class="typeColor(node.outputType)" class="text-xs py-1 px-2 leading-none dark:bg-gray-900 rounded-md">{{ node.outputType }}</div>
				<!-- <input @pointerdown.stop type="text" class="pl-8 h-9 bg-transparent border border-gray-300 dark:border-gray-700 dark:text-white w-full rounded-md text-sm" placeholder="Search"></input> -->
			</div>
			<div class="h-full node__out">
				<point-out :id="node.uuid" :type="node.outputType" :class="typePointColor(node.outputType)" class="w-2 h-2 border leading-none rounded-full"/>
			</div>
		</div>

		<div v-for="(item, index) in node!.input" :root-id="node.uuid" class="flex-initial h-12 px-3 xl:border-t border-gray-200 dark:border-gray-700 relative">
