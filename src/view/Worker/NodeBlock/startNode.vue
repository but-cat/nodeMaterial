<template>
	<node-block :position="`${node!.x},${node!.y}`" :id="`block-${node.uuid}`" @click="emitActiveNode" @connected-down="emit('down', $event)" @connected-up="emit('up')" @connected-update="positionUpdate" :key="`start-${node.uuid}`" :class="isActive ? ['ring-2', 'ring-primary-300'] : []" ref="nodeBlock" class="w-64 bg-white flex flex-col rounded-md dark:bg-gray-800 shadow-lg">
